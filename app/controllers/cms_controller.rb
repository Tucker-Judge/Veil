class CmsController < ApplicationController
    ...
    def vocab
        ...
        params[:back_arr].each_with_index do |sub,i|
            ...
            # card_type and title with translation options
            set = FlashcardSet.create(language_id: language.id, card_type: params[:card_type].shift, title: params[:title].shift)
            locale = params[:translations].shift.to_sym
            sub.each |n| do
                ...
                if Flashcard.find_by(front: front_card).exists?
                    I18n.with_locale(locale) {card.back = back_card}
                else
...
def authenticate_admin
    unless current_user.admin?
        render json: {error: "what a drag"},status: :unauthorized
    end
end
